...
func parseCmd(args []string) (string, int64, error) {
    ...
    if l > 2 {
        return "", 0, errors.New(fmt.Sprintf("Too many args provided %v, usage: jspand <filepath> <desired number of keys>[optional]", args))
    }
...
func main() {
    ...
    filepath, size, err := parseCmd(testCMD)
    if err != nil {
        fmt.Println(err)
    }
    ...
    if fs := f.Size(); fs > size {
        fmt.Println("file size (%v bytes) is larger than max byte size (%v bytes)", fs, size)
        os.Exit(1)
    } 
